...
parser.add_argument("--model_path", default='roberta-large', type=str)
...
class LMForwardAPI:
    def __init__(self, args, model_name='roberta-large', model_path='transformer_model/roberta-large', n_prompt_tokens=50, task_name='sst2',
        ...
        if model_name in ['roberta-base', 'roberta-large']:
            ...
            self.config = CPTConfig.from_pretrained(model_path)
            self.tokenizer = BertTokenizer.from_pretrained(model_path)
        ...
        # for student model
        if args.student_model_name in ['bert-base-uncased', 'bert-large-uncased']:
            self.stu_config = BertConfig.from_pretrained(args.student_model_path)
            self.stu_tokenizer = BertTokenizer.from_pretrained(args.student_model_path)
            self.stu_model = BertForMaskedLM.from_pretrained(
    ...
    def eval(self, prompt_embedding=None, test_data=None):
        ...
        self.stu_model.set_prompt_embedding(prompt_embedding)

        # TO DO: 把此处的prompt_embedding[32, 50, 1024]存下来作为训练集中的X
